...
const zhRustNote = defineNoteConfig({
  dir: 'rust',
  ...
  link: '/rust',
  // 手动配置侧边栏结构
  sidebar: ['', 'quickstart', 'article1', 'article2', 'article3', 'article4'],
  // 根据文件结构自动生成侧边栏
  // sidebar: 'auto',
})
const zhEngineeringNote = defineNoteConfig({
  dir: 'engineering',
  // `dir` 所指向的目录中的所有 markdown 文件，其 permalink 需要以 `link` 配置作为前缀
  // 如果 前缀不一致，则无法生成侧边栏。
  // 所以请确保  markdown 文件的 permalink 都以 `link` 开头
...
export const zhNotes = defineNotesConfig({
  dir: 'notes',
  link: '/',
  notes: [zhRustNote, zhEngineeringNote],
})
